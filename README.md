...
<body>
    <div class="container">
        ...
        <div class="tabs">
            ...
            <button class="tab" onclick="switchTab('iptracker')">🔍 IP Tracker</button>
            <button class="tab" onclick="switchTab('tools')">🛠️ Outils</button>
        </div>
        ...
        <div id="dashboard" class="tab-content active">
            ...
            <div class="card">
                ...
                <button class="btn" onclick="sendQuickBroadcast()">📤 Broadcast Rapide</button>
                <button class="btn btn-success" onclick="generateInviteLink()">🔗 Générer un Lien</button>
                <button class="btn btn-secondary" onclick="exportUsers()">📥 Exporter Utilisateurs</button>
            </div>

            <div class="card">
                <h2>🌐 Votre Appareil</h2>
        ...
        <div id="broadcast" class="tab-content">
            <div class="alert">
                ⚠️ Le message sera envoyé à tous les utilisateurs actifs
            </div>

            <div class="card">
                <h2>📢 Nouveau Broadcast</h2>
                <div class="input-group">
                    <label>Message</label>
                ...
                <div class="input-group">
                    ...
                    <select id="broadcastType">
                        ...
                        <option value="withimage">🖼️ Avec image</option>
                        <option value="urgent">⚠️ Message urgent</option>
                    </select>
                </div>
                <button class="btn btn-warning" onclick="previewBroadcast()">👁️ Prévisualiser</button>
                <button class="btn" onclick="confirmBroadcast()">📤 Envoyer</button>
            </div>
        </div>
        ...
        <div id="iptracker" class="tab-content">
            <div class="alert">
                ⚠️ <strong>Rappel légal :</strong> Informez toujours l'utilisateur de la collecte d'IP
            </div>

            <div class="card">
                <h2>🔍 Votre IP Actuelle</h2>
                <div class="ip-info" id="myIpInfo">
                    <div class="loading">
                        <div class="spinner"></div>
                        Récupération de votre IP...
                    </div>
                </div>
            </div>

            <div class="card">
                <h2>🌐 Générer un Lien de Tracking</h2>
                <p>Utilisez ces services avec transparence</p>
                <button class="btn" onclick="openGrabify()">🌐 Ouvrir Grabify</button>
                <button class="btn btn-secondary" onclick="openIPLogger()">📊 Ouvrir IPLogger</button>
                <button class="btn btn-success" onclick="openIPify()">🔍 Ouvrir IPify</button>
            </div>

            <div class="card">
                <h2>📋 Template de Message Légal</h2>
                <textarea readonly style="font-size: 12px; background: #f8f9fa;">🔐 Vérification de sécurité

Cliquez sur ce lien pour vérifier votre compte :
[VOTRE_LIEN_ICI]

⚠️ IMPORTANT : Ce lien collecte votre adresse IP pour des raisons de sécurité et de prévention de fraude.

En cliquant, vous acceptez la collecte de :
• Adresse IP
• Localisation approximative
• Type d'appareil</textarea>
                <button class="btn btn-success" onclick="copyTemplate()" style="margin-top: 10px;">📋 Copier le Template</button>
            </div>
        </div>
        ...
        <div id="tools" class="tab-content">
            <div class="card">
                ...
                <button class="btn" onclick="banUser()">🚫 Bannir un Utilisateur</button>
                <button class="btn btn-success" onclick="unbanUser()">✅ Débannir un Utilisateur</button>
                <button class="btn btn-secondary" onclick="viewInfo()">ℹ️ Info Utilisateur</button>
            </div>

            <div class="card">
                <h2>📊 Statistiques Avancées</h2>
                <button class="btn" onclick="viewStats()">📈 Voir les Stats Complètes</button>
                <button class="btn btn-warning" onclick="clearCache()">🗑️ Vider le Cache</button>
            </div>

            <div class="card">
                <h2>⚙️ Paramètres</h2>
                <button class="btn btn-secondary" onclick="openBotSettings()">🤖 Paramètres du Bot</button>
                <button class="btn" onclick="reloadData()">🔄 Recharger les Données</button>
            </div>
        </div>
    </div>

    <div class="footer">
        Développé avec ❤️ pour Telegram • © 2026
    </div>
    ...
    <script>
        ...
        // Récupérer les infos utilisateur Telegram
        const user = tg.initDataUnsafe.user;
        if (user) {
            document.getElementById('userInfo').textContent = 
                `Admin: ${user.first_name} (@${user.username || 'N/A'}) • ID: ${user.id}`;
        }
        ...
        // Détecter les informations de l'appareil avec User Agent
        function detectDevice() {
            ...
            else if (ua.indexOf('Tablet') > -1) deviceType = '📱 Tablette';
            ...
            const deviceInfo = `
                ...
                <div class="ip-info-row">
                    <span class="ip-label">Système</span>
                ...
                <div class="ip-info-row">
                    <span class="ip-label">Écran</span>
        ...
        function loadUsers() {
            ...
            mockUsers.forEach(user => {
                const statusEmoji = user.status === 'verified' ? '✅' : user.status === 'banned' ? '🚫' : '👤';
                html += `
                    <div class="user-item">
                        <div class="user-info-item">
                            <div class="user-name">${statusEmoji} ${user.name}</div>
                            <div class="user-id">@${user.username} • ID: ${user.id}</div>
                        </div>
                        <div class="user-actions">
                            <button class="btn btn-small" onclick="viewUserInfo(${user.id})">ℹ️</button>
                            <button class="btn btn-small btn-secondary" onclick="banUserById(${user.id})">🚫</button>
                        </div>
                    </div>
                `;
            });
        ...
        function generateInviteLink() {
            tg.showAlert('Utilisez /admin dans le bot pour générer un lien d\'invitation');
        }

        function exportUsers() {
            tg.showPopup({
                title: '📥 Export en cours',
                message: 'Les données seront exportées via le bot. Vérifiez vos messages privés.',
                buttons: [{type: 'ok'}]
            });
        }
        ...
        function searchUser() {
            ...
            tg.showAlert(`Recherche de: ${query}\n\nUtilisez /info ${query} dans le bot pour voir les détails`);
        }
        ...
        function confirmBroadcast() {
            ...
            tg.showConfirm('Envoyer ce broadcast à tous les utilisateurs ?', (confirmed) => {
                if (confirmed) {
                    tg.showAlert('Broadcast envoyé ! Vérifiez les résultats dans le bot avec /admin');
                }
            });
        }
        ...
        function viewStats() {
            tg.showAlert('Consultez les statistiques complètes avec /admin dans le bot');
        }
        ...
        function clearCache() {
            tg.showConfirm('Vider le cache local ?', (confirmed) => {
                if (confirmed) {
                    localStorage.clear();
                    tg.showAlert('✅ Cache vidé !');
                }
            });
        }

        function openBotSettings() {
            tg.showAlert('Paramètres disponibles via le bot avec /admin');
        }
        ...
        function reloadData() {
            ...
            detectDevice();
            getPublicIP();
            tg.showAlert('✅ Données rechargées !');
        }

        function viewUserInfo(userId) {
            tg.showAlert(`Utilisez /info ${userId} dans le bot pour voir les détails complets`);
        }
